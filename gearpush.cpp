...
int main(int argc, char** argv) {
    ros::init(argc, argv, "gearpush"); // name this node 
    ros::NodeHandle nh; //standard ros node handle   
    Eigen::Affine3d start_flange_affine, goal_flange_affine,desired_flange_affine; //specify start and goal in Cartesian coords
    ...
    Eigen::Vector3d x_axis, y_axis, z_axis, flange_origin, desired_origin;
    ...
    int nsteps = 50; //will need to specify how many interpolation points in Cartesian path; this is pretty coarse
    double arrival_time = 3.0; //will  need to specify arrival time for a Cartesian path
    ...
    do
    {
        cout<<"Enter X,Y-coordinates for the desired location: ";//ask for user input for desired gear part location
        cin>>desired.pose.position.x;
        cin>>desired.pose.position.y;
  

        desired_origin<<desired.pose.position.x, desired.pose.position.y,0;
        desired_flange_affine.translation()=desired_origin;
        nsteps = 50;//arbitrary  
        optimal_path.clear(); 
        //ROS_WARN("Desired object position not within workspace. Re-enter the values.");    
    } while (!pCartTrajPlanner->plan_cartesian_path_w_rot_interp(g_q_vec_arm_Xd, desired_flange_affine,
            nsteps, optimal_path));//check to see if the desired location is in the workspace of this robot

    bool cont=true;//a boolean to check if the gear part is within a small radius of the designated location; if not, continue the pushing process
    ...
    flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y, 0.3; //specify coordinates for the desired flange position (origin) with respect to the robot's base frame
    ...
    //depending on the discrepancy between the desired location and the current location of the gera part, go to pose to push in correct direction
    if (g_perceived_object_pose.pose.position.x<desired.pose.position.x && g_perceived_object_pose.pose.position.y<desired.pose.position.y)
    {
        flange_origin <<g_perceived_object_pose.pose.position.x-0.05, g_perceived_object_pose.pose.position.y-0.05, 0.003; 

    }
    else if (g_perceived_object_pose.pose.position.x>desired.pose.position.x && g_perceived_object_pose.pose.position.y<desired.pose.position.y)
    {
        flange_origin <<g_perceived_object_pose.pose.position.x+0.05, g_perceived_object_pose.pose.position.y-0.05, 0.003; 
    ...
    flange_origin <<desired.pose.position.x, desired.pose.position.y, 0; 
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    ...
    flange_origin << desired.pose.position.x, desired.pose.position.y, 0.3;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    ...
    ROS_INFO("Finished final trajectory.");

    object_finder_ac.sendGoal(object_finder_goal,&objectFinderDoneCb);//update the current location of the gear part
    float x_offset=abs(g_perceived_object_pose.pose.position.x-desired.pose.position.x);//get absolute differences between the desired and current location of the part
    float y_offset=abs(g_perceived_object_pose.pose.position.y-desired.pose.position.y);

    if (x_offset>=0.05 || y_offset>=0.05)//if the difference is greater than 0.05, continue this process
    {
        cont=true;
    }
    else{
